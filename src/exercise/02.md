...
## 📝 Your Notes
- React.createElement returns Element which is DOM API's Element
- React.createElement accesspts three params , type : a string of element type , props , an Object for props and rootContainerElement which can be element type, document or documentFragment type
- if the props is empty you can set it to null or {} . This object can also contain children which if has only one value can be onject or needs to be array if more than on children esists
- samples `React.createElement('h1', {}, 'Hello World')`
- multiple ones 
```
